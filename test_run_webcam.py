...
# Initialize images to be displayed on Streamlit
st_frame = st.empty()

# Function to retrieve a frame image from the camera
def get_frame():
    _, frame = cap.read()
    bgr = cv2.cvtColor(frame, cv2.COLOR_BGR)
    return bgr

def estimate_gaze(frame):
    print("running")
    bgr = cv2.cvtColor(frame, cv2.COLOR_BGR)
    faces = face_cascade.detectMultiScale(bgr)
    ...
    if current_face is not None:
            ...
            next_landmarks = detect_landmarks(current_face, bgr)
    ...
    if landmarks is not None:
            eye_samples = segment_eyes(bgr, landmarks)
            ...
            print("left eye: ", (left_eye._gaze)*(180/math.pi))
            print("right eye: ", (right_eye._gaze)*(180/math.pi))
...
# Loop to display camera stream on Streamlit
while True:
    # FPSの設定
    interval = 1.0 / fps
